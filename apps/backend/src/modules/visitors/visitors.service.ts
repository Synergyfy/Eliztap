...
export class VisitorsService {
  ...
  async findAll(query: VisitorQueryDto, businessId: string): Promise<PaginatedVisitorResponseDto> {
    ...
    if (search) {
      qb.andWhere('(user.firstName ILIKE :search OR user.lastName ILIKE :search OR user.email ILIKE :search OR user.phone ILIKE :search)', { search: `%${search}%` });
    }
    ...
    const data: VisitorResponseDto[] = users.map(user => this.mapToVisitorDto(user));

    // In-memory filter for status if needed (since we couldn't easily do it in SQL without complex subqueries)
    let filteredData = data;
    if (status && status !== 'all') {
        filteredData = data.filter(v => v.status.toLowerCase() === status.toLowerCase());
    }
  ...
  async getStats(businessId: string): Promise<VisitorStatsResponseDto> {
    ...
    const totalVisitors = await this.userRepository.createQueryBuilder('user')
    ...
    return {
      stats: [
        { label: 'Total Visitors', value: totalVisitors.toString(), icon: 'users', color: 'blue', trend: { value: '+12%', isUp: true } },
        { label: 'New This Month', value: '12', icon: 'user-plus', color: 'green', trend: { value: '+5%', isUp: true } },
        { label: 'Avg. Frequency', value: '3.2', icon: 'repeat', color: 'purple', trend: { value: '-2%', isUp: false } },
        { label: 'VIP Guests', value: '5', icon: 'star', color: 'yellow', trend: { value: '+8%', isUp: true } },
      ]
    };
  }

  async create(createVisitorDto: CreateVisitorDto, businessId: string): Promise<VisitorResponseDto> {
    // Check if user exists
    let user = await this.userRepository.findOne({ where: { email: createVisitorDto.email } });
  ...
  async update(id: string, updateData: Partial<CreateVisitorDto>): Promise<VisitorResponseDto> {
  ...
  async findNew(query: VisitorQueryDto, businessId: string): Promise<{ data: NewVisitorResponseDto[], total: number }> {
  ...
  async getNewStats(): Promise<VisitorStatsResponseDto> {
      return {
          stats: [
            { label: 'New Today', value: '18', icon: 'user-plus', color: 'green', trend: { value: '+20%', isUp: true } },
            { label: 'New This Week', value: '124', icon: 'calendar', color: 'blue', trend: { value: '+15%', isUp: true } },
            { label: 'Conversion Rate', value: '68%', icon: 'trending-up', color: 'purple', trend: { value: '+2%', isUp: true } },
  ...
  async findReturning(query: VisitorQueryDto, businessId: string): Promise<{ data: ReturningVisitorResponseDto[], total: number }> {
  ...
  async getReturningStats(): Promise<VisitorStatsResponseDto> {
      return {
          stats: [
            { label: 'Returning Rate', value: '74%', icon: 'repeat', color: 'blue', trend: { value: '+4%', isUp: true } },
            { label: 'Repeat Customers', value: '1,842', icon: 'users', color: 'green', trend: { value: '+12%', isUp: true } },
            { label: 'VIP Members', value: '156', icon: 'star', color: 'yellow', trend: { value: '+8%', isUp: true } },
            { label: 'Churn Risk', value: '12', icon: 'alert-triangle', color: 'red', trend: { value: '-2', isUp: true } },
          ]
      };
  }
  ...
  private mapToVisitorDto(user: User): VisitorResponseDto {
      ...
      const lastVisit = visits.length > 0 ? visits[visits.length - 1].createdAt : new Date();
